//! # 配置管理命令
//!
//! 实现 `orangecoding config` 子命令，用于查看和管理 OrangeCoding 的配置。
...
use orangecoding_config::ConfigManager;
...
/// 不支持的键名会打印警告信息。
fn apply_config_value(config: &mut orangecoding_config::OrangeConfig, key: &str, value: &str) {
    match key {
        // AI 相关配置
        ...
        _ => {
            ...
            eprintln!("   使用 'orangecoding config show' 查看所有可用的配置项");
        }
    }
}
...
async fn get_config(config_manager: &ConfigManager, key: &str) -> Result<()> {
    ...
    let value = match key {
        ...
        _ => {
            anyhow::bail!(
                "未知的配置键: '{}'\n使用 'orangecoding config show' 查看所有可用的配置项",
                key
            );
        }
    };
...
async fn init_config(config_manager: &ConfigManager, force: bool) -> Result<()> {
    ...
    println!("  2. 设置 API 密钥: orangecoding config set ai.api_key YOUR_KEY");
    println!("  3. 设置提供商:    orangecoding config set ai.provider deepseek");
    println!("  4. 查看配置:      orangecoding config show");
