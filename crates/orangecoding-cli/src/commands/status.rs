//! # 状态查看命令
//!
//! 实现 `orangecoding status` 子命令，用于显示系统的运行状态信息。
//! 包括当前配置概要、AI 提供商状态、已注册工具列表和版本信息。

use anyhow::Result;
use clap::Args;

use orangecoding_config::OrangeConfig;
...
/// - `args`: 状态命令参数
/// - `config`: 当前生效的配置
pub async fn execute(args: StatusArgs, config: OrangeConfig) -> Result<()> {
    // 输出版本信息
...
///
/// 显示当前使用的提供商、模型以及相关参数。
fn print_provider_section(config: &OrangeConfig) {
    println!("🤖 AI 提供商");
    println!("{}", "─".repeat(50));
...
/// 密钥内容不会被显示，只报告是否可用。
fn print_api_key_section(config: &OrangeConfig) {
    println!("🔑 API 密钥状态");
    println!("{}", "─".repeat(50));
...
fn print_config_section(config: &OrangeConfig) {
    println!("⚙️  配置摘要");
    println!("{}", "─".repeat(50));
    ...
    // 配置文件位置提示
    println!();
    println!("  提示: 使用 'orangecoding config init' 初始化配置文件");
