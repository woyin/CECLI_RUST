//! # Serve 命令
//!
//! 实现 `orangecoding serve` 子命令，启动本地 HTTP + WebSocket 控制服务器。
...
use orangecoding_agent::executor::ToolExecutor;
use orangecoding_agent::{AgentContext, AgentLoop, AgentLoopConfig};
use orangecoding_ai::{AiProvider, ChatOptions, ProviderConfig, ProviderFactory};
use orangecoding_config::{ModelsConfig, OrangeConfig};
use orangecoding_control_server::{start_server, ControlServerConfig};
use orangecoding_core::{AgentEvent, AgentId, SessionId};
use orangecoding_tools::{create_default_registry, SecurityPolicy};
use orangecoding_worker::{AgentExecutor, WorkerRuntime};
...
pub async fn execute(args: ServeArgs, config: OrangeConfig) -> Result<()> {
    info!("正在启动控制服务器...");
...
fn setup_provider(config: &OrangeConfig) -> Result<Box<dyn AiProvider>> {
