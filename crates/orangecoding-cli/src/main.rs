...
/// OrangeCoding — 一个 AI 驱动的编码智能体 CLI 工具
#[derive(Parser, Debug)]
#[command(
    name = "orangecoding",
    version,
...
struct Cli {
    /// 启动参数（无子命令时直接生效，等同于 orangecoding launch）
    #[command(flatten)]
...
async fn main() -> Result<()> {
    ...
    // 加载配置
    let config_manager = orangecoding_config::ConfigManager::new()
...
async fn dispatch_command(
    command: Commands,
    config: orangecoding_config::OrangeConfig,
    config_manager: orangecoding_config::ConfigManager,
) -> Result<()> {
    match command {
