//! # 项目初始化命令
//!
//! 实现 `ceair init` 子命令，用于初始化项目的 CEAIR 配置。
...
"#
    ...
    pub fn files_to_create(options: &InitOptions) -> Vec<(PathBuf, String)> {
        ...
        if options.create_config {
            files.push((
                dir.join(".ceair").join("config.toml"),
        ...
        if options.create_commands_dir {
            // 命令目录用 .gitkeep 占位
            files.push((
                dir.join(".ceair").join("commands").join(".gitkeep"),
                String::new(),
            ));
        }
    ...
    pub fn is_initialized(project_dir: &Path) -> bool {
        project_dir.join(".ceair").join("config.toml").exists()
    }
}
...
mod tests {
    ...
    fn test_is_initialized_with_config() {
        let dir = tempfile::tempdir().unwrap();
        let ceair_dir = dir.path().join(".ceair");
        std::fs::create_dir_all(&ceair_dir).unwrap();
        std::fs::write(ceair_dir.join("config.toml"), "# config").unwrap();
