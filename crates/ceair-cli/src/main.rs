...
/// CEAIR — 一个 AI 驱动的编码智能体 CLI 工具
#[derive(Parser, Debug)]
#[command(
    name = "ceair",
    version,
    about = "CEAIR — AI 驱动的编码智能体命令行工具",
    long_about = "CEAIR 是一个基于 AI 的编码助手 CLI 工具，\n\
...
async fn main() -> Result<()> {
    ...
    // 加载配置
    let config_manager = ceair_config::ConfigManager::new()
...
async fn dispatch_command(
    command: Commands,
    config: ceair_config::CeairConfig,
    config_manager: ceair_config::ConfigManager,
) -> Result<()> {
    match command {
