...
impl ModelRoleRouter {
    ...
    ///
    /// 支持的环境变量：
    /// - `CEAIR_MODEL_DEFAULT` — 默认模型
    /// - `CEAIR_MODEL_SMOL` — 小模型
    /// - `CEAIR_MODEL_SLOW` — 慢模型
    /// - `CEAIR_MODEL_PLAN` — 计划模型
    /// - `CEAIR_MODEL_COMMIT` — 提交模型
    ///
    ...
    pub fn from_env() -> Self {
        ...
        let env_mappings = [
            ("CEAIR_MODEL_DEFAULT", ModelRole::Default),
            ("CEAIR_MODEL_SMOL", ModelRole::Smol),
            ("CEAIR_MODEL_SLOW", ModelRole::Slow),
            ("CEAIR_MODEL_PLAN", ModelRole::Plan),
            ("CEAIR_MODEL_COMMIT", ModelRole::Commit),
        ];
...
mod tests {
    ...
    fn test_from_env() {
        // 设置环境变量，验证从环境变量加载
        std::env::set_var("CEAIR_MODEL_DEFAULT", "openai/gpt-4o:medium");
        std::env::set_var("CEAIR_MODEL_SMOL", "gpt-4o-mini");
        ...
        // 清理环境变量
        std::env::remove_var("CEAIR_MODEL_DEFAULT");
        std::env::remove_var("CEAIR_MODEL_SMOL");
    }
}
