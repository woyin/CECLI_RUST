//! # 主配置系统
//!
//! 定义 ChengCoding 的所有配置结构体，以及用于加载、保存和管理配置的 `ConfigManager`。
...
impl CeairConfig {
    ...
    /// 将配置序列化为 TOML 字符串
    pub fn to_toml(&self) -> chengcoding_core::Result<String> {
        toml::to_string_pretty(self)
            .map_err(|e| CeairError::serialization(format!("TOML 序列化失败: {e}")))
    }

    /// 将配置序列化为 JSON 字符串
    pub fn to_json(&self) -> chengcoding_core::Result<String> {
...
impl ConfigManager {
    ...
    pub fn with_dir(dir: PathBuf) -> chengcoding_core::Result<Self> {
    ...
    pub async fn load(&self) -> chengcoding_core::Result<CeairConfig> {
    ...
    /// 将当前配置保存到磁盘
    pub async fn save(&self) -> chengcoding_core::Result<()> {
        let config = self.config.read().await;
        self.save_config(&config).await
    }

    /// 将指定配置写入磁盘
    async fn save_config(&self, config: &CeairConfig) -> chengcoding_core::Result<()> {
        // 确保配置目录存在
    ...
    ///
    /// 闭包接收可变引用，修改后自动保存到磁盘。
    pub async fn update<F>(&self, updater: F) -> chengcoding_core::Result<()>
    where
    ...
    /// 重新从磁盘加载配置（热重载）
    pub async fn reload(&self) -> chengcoding_core::Result<CeairConfig> {
        info!("重新加载配置...");
        self.load().await
    }
    ...
    pub async fn merge_from_file(&self, path: &Path) -> chengcoding_core::Result<()> {
        info!("从文件合并配置: {:?}", path);
