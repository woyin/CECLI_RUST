...
pub enum ConfigSource {
    ...
    /// 从环境变量加载（如 OrangeCoding_AI_PROVIDER）
    Environment,
...
impl ConfigLayer {
    ...
    /// 从 OrangeConfig 构建配置层
    ///
    /// 将结构体扁平化为点分隔的键值对。
    pub fn from_config(
        source: ConfigSource,
        config: &OrangeConfig,
    ) -> orangecoding_core::Result<Self> {
        let mut layer = Self::new(source);

        // 将配置序列化为 TOML 值，然后扁平化
        let toml_str = config.to_toml()?;
        let toml_value: toml::Value = toml::from_str(&toml_str)
            .map_err(|e| OrangeError::config(format!("配置序列化失败: {e}")))?;
...
impl LayeredConfig {
    ...
    ///
    /// 从默认配置开始，依次用每个层的值覆盖，
    /// 最终得到一个完整的 `OrangeConfig`。
    pub fn get_effective_config(&self) -> orangecoding_core::Result<OrangeConfig> {
        ...
        let toml_str = toml::to_string_pretty(&nested)
            .map_err(|e| OrangeError::serialization(format!("合并配置序列化失败: {e}")))?;

        OrangeConfig::from_toml(&toml_str)
    }
...
mod tests {
    ...
    fn test_source_display_name() {
        ...
        let file_src = ConfigSource::File(PathBuf::from("/etc/orangecoding/config.toml"));
