...
impl ModelsConfig {
    pub fn load_from_file(path: &Path) -> orangecoding_core::Result<Self> {
        let content = std::fs::read_to_string(path).map_err(|e| {
            OrangeError::config(format!("读取模型配置文件失败: {}: {}", path.display(), e))
        })?;

        serde_yaml::from_str(&content).map_err(|e| {
            OrangeError::config(format!("解析模型配置 YAML 失败: {}: {}", path.display(), e))
        })
    }
    ...
    /// 返回已知提供商的预定义配置（包含模型列表和 base_url）
    ...
    /// 将用户配置与预定义配置合并：用户只需提供 api_key 即可自动补全其余字段
    pub fn merge_with_predefined(&mut self) {
        let known_providers = ["zai", "zen"];
        for name in &known_providers {
            if let Some(user_config) = self.providers.get_mut(*name) {
                if let Some(predefined) = Self::predefined_provider_config(name) {
...
"#,
    ...
    // -----------------------------------------------------------------------
    // 10. test_predefined_provider_configs
    // -----------------------------------------------------------------------
    #[test]
    fn test_predefined_zai_models() {
        ...
        assert_eq!(config.base_url.as_deref(), Some("https://api.z.ai/v1"));
        assert!(!config.models.is_empty());
        assert_eq!(config.api, Some(ApiType::OpenAiCompletions));
        assert_eq!(config.auth, Some(AuthType::Bearer));
    }
    ...
    fn test_predefined_unknown_provider() {
        assert!(ModelsConfig::predefined_provider_config("unknown").is_none());
    }

    // -----------------------------------------------------------------------
    // 11. test_merge_with_predefined
    // -----------------------------------------------------------------------
    #[test]
    fn test_merge_with_predefined() {
        ...
        assert_eq!(zai.api, Some(ApiType::OpenAiCompletions));
    }

    #[test]
    fn test_merge_preserves_user_overrides() {
        let mut config = ModelsConfig::default();
        let custom_model = ModelDefinition {
            id: "my-custom-z1".to_string(),
            name: Some("Custom Z1".to_string()),
            reasoning: None,
            input: None,
            cost: None,
            context_window: None,
        ...
        config.providers.insert(
            "zai".to_string(),
            ProviderConfig {
                base_url: Some("https://my-proxy.com/v1".to_string()),
                api_key: Some("my-key".to_string()),
                api: Some(ApiType::OpenAiResponses),
                headers: None,
                auth: Some(AuthType::ApiKey),
        ...
        assert_eq!(zai.base_url.as_deref(), Some("https://my-proxy.com/v1"));
        assert_eq!(zai.api, Some(ApiType::OpenAiResponses));
        assert_eq!(zai.auth, Some(AuthType::ApiKey));
        assert_eq!(zai.models.len(), 1);
        assert_eq!(zai.models[0].id, "my-custom-z1");
    }
}
