//! # 主配置系统
//!
//! 定义 OrangeCoding 的所有配置结构体，以及用于加载、保存和管理配置的 `ConfigManager`。
...
use orangecoding_core::OrangeError;
...
impl OrangeConfig {
    /// 从 TOML 字符串解析配置
    pub fn from_toml(content: &str) -> orangecoding_core::Result<Self> {
        toml::from_str(content).map_err(|e| OrangeError::config(format!("TOML 解析失败: {e}")))
    }

    /// 将配置序列化为 TOML 字符串
    pub fn to_toml(&self) -> orangecoding_core::Result<String> {
        toml::to_string_pretty(self)
            .map_err(|e| OrangeError::serialization(format!("TOML 序列化失败: {e}")))
    }

    /// 将配置序列化为 JSON 字符串
    pub fn to_json(&self) -> orangecoding_core::Result<String> {
        serde_json::to_string_pretty(self)
            .map_err(|e| OrangeError::serialization(format!("JSON 序列化失败: {e}")))
    }
}
...
pub struct ConfigManager {
    /// 当前生效的配置（线程安全的读写锁）
    config: Arc<RwLock<OrangeConfig>>,
...
impl ConfigManager {
    /// 创建新的配置管理器
    ///
    /// 自动检测 XDG 配置目录（~/.config/orangecoding/），如果不存在则创建。
    pub fn new() -> orangecoding_core::Result<Self> {
        ...
        Ok(Self {
            config: Arc::new(RwLock::new(OrangeConfig::default())),
            config_dir,
            config_path,
        })
    }
    ...
    pub fn with_dir(dir: PathBuf) -> orangecoding_core::Result<Self> {
    ...
    /// 解析配置目录路径
    ///
    /// 始终使用 `~/.config/orangecoding` 作为配置目录，
    /// 遵循 XDG 规范，不使用 macOS 的 `~/Library/Application Support`。
    fn resolve_config_dir() -> orangecoding_core::Result<PathBuf> {
        let home = dirs::home_dir().ok_or_else(|| OrangeError::config("无法确定用户主目录"))?;
        let new_dir = home.join(".config").join("orangecoding");
        if new_dir.exists() {
            return Ok(new_dir);
        }
        let legacy_dir = home.join(".config").join("orangecoding");
    ...
    pub async fn load(&self) -> orangecoding_core::Result<OrangeConfig> {
        if self.config_path.exists() {
            info!("从文件加载配置: {:?}", self.config_path);
            let content = tokio::fs::read_to_string(&self.config_path)
                .await
                .map_err(OrangeError::from)?;
            let loaded = OrangeConfig::from_toml(&content)?;
            ...
            *config = loaded.clone();
            ...
            info!("配置文件不存在，使用默认配置");
            let default_config = OrangeConfig::default();
    ...
    /// 将当前配置保存到磁盘
    pub async fn save(&self) -> orangecoding_core::Result<()> {
        let config = self.config.read().await;
        self.save_config(&config).await
    }

    /// 将指定配置写入磁盘
    async fn save_config(&self, config: &OrangeConfig) -> orangecoding_core::Result<()> {
        // 确保配置目录存在
        if !self.config_dir.exists() {
            tokio::fs::create_dir_all(&self.config_dir)
                .await
                .map_err(|e| OrangeError::io(format!("创建配置目录失败: {e}")))?;
        }
    ...
    /// 获取当前配置的只读副本
    pub async fn get(&self) -> OrangeConfig {
    ...
    ///
    /// 闭包接收可变引用，修改后自动保存到磁盘。
    pub async fn update<F>(&self, updater: F) -> orangecoding_core::Result<()>
    where
        F: FnOnce(&mut OrangeConfig),
    {
        {
            let mut config = self.config.write().await;
            updater(&mut config);
    ...
    /// 重新从磁盘加载配置（热重载）
    pub async fn reload(&self) -> orangecoding_core::Result<OrangeConfig> {
        info!("重新加载配置...");
        self.load().await
    }
    ...
    pub async fn merge_from_file(&self, path: &Path) -> orangecoding_core::Result<()> {
        info!("从文件合并配置: {:?}", path);
        ...
        let override_value: toml::Value = toml::from_str(&content)
            .map_err(|e| OrangeError::config(format!("合并文件解析失败: {e}")))?;
        ...
        let mut current_value: toml::Value = toml::from_str(&current_toml)
            .map_err(|e| OrangeError::config(format!("当前配置序列化失败: {e}")))?;
        ...
        let merged_toml = toml::to_string_pretty(&current_value)
            .map_err(|e| OrangeError::serialization(format!("合并结果序列化失败: {e}")))?;
        *config = OrangeConfig::from_toml(&merged_toml)?;
...
mod tests {
    ...
    fn test_default_config() {
        let config = OrangeConfig::default();
    ...
    fn test_toml_roundtrip() {
        let original = OrangeConfig::default();
        let toml_str = original.to_toml().expect("序列化失败");
        let parsed = OrangeConfig::from_toml(&toml_str).expect("反序列化失败");
...
max_iterations = 100
"#;
        let config = OrangeConfig::from_toml(partial_toml).expect("部分配置解析失败");
