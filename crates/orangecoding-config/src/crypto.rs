...
impl CryptoStore {
    ...
    /// 从默认配置目录创建加密存储器
    ///
    /// 存储文件位于 ~/.config/orangecoding/secrets.json
    pub fn default_store() -> orangecoding_core::Result<Self> {
    ...
    pub fn encrypt_value(
        ...
        plaintext: &str,
        passphrase: &str,
    ) -> orangecoding_core::Result<Vec<u8>> {
        debug!("开始加密数据");
        ...
        self.rng
            .fill(&mut salt)
            .map_err(|_| OrangeError::internal("生成随机盐值失败"))?;

        // 生成随机 Nonce
        let mut nonce_bytes = [0u8; NONCE_LEN];
        self.rng
            .fill(&mut nonce_bytes)
            .map_err(|_| OrangeError::internal("生成随机 Nonce 失败"))?;
        ...
        let unbound_key = aead::UnboundKey::new(&aead::AES_256_GCM, &key_bytes)
            .map_err(|_| OrangeError::internal("创建加密密钥失败"))?;
        ...
        sealing_key
            ...
            .map_err(|_| OrangeError::internal("加密操作失败"))?;
        ...
        serde_json::to_vec(&encrypted)
            .map_err(|e| OrangeError::serialization(format!("加密数据序列化失败: {e}")))
    }
    ...
    pub fn decrypt_value(
        ...
        encrypted_bytes: &[u8],
        passphrase: &str,
    ) -> orangecoding_core::Result<String> {
        debug!("开始解密数据");

        // 反序列化加密数据包
        let encrypted: EncryptedData = serde_json::from_slice(encrypted_bytes)
            .map_err(|e| OrangeError::serialization(format!("加密数据反序列化失败: {e}")))?;
        ...
        let nonce_bytes: [u8; NONCE_LEN] = encrypted
            ...
            .as_slice()
            .try_into()
            .map_err(|_| OrangeError::internal("Nonce 长度无效"))?;

        // 创建解密器
        let unbound_key = aead::UnboundKey::new(&aead::AES_256_GCM, &key_bytes)
            .map_err(|_| OrangeError::internal("创建解密密钥失败"))?;
        let nonce_seq = OneNonceSequence::new(nonce_bytes);
        ...
        let plaintext = opening_key
            ...
            .map_err(|_| OrangeError::internal("解密失败，口令可能不正确"))?;

        String::from_utf8(plaintext.to_vec())
            .map_err(|e| OrangeError::internal(format!("解密结果不是有效的 UTF-8: {e}")))
    }
    ...
    pub fn retrieve_secret(
        ...
        let encrypted = store
            .secrets
            .get(name)
            .ok_or_else(|| OrangeError::config(format!("密钥 '{}' 不存在", name)))?;
    ...
    fn encrypt_to_data(
        ...
        plaintext: &str,
        passphrase: &str,
    ) -> orangecoding_core::Result<EncryptedData> {
        // 生成随机盐值
        let mut salt = [0u8; SALT_LEN];
        self.rng
            .fill(&mut salt)
    ...
    /// 从文件加载密钥存储
    fn load_store(&self) -> orangecoding_core::Result<SecretStore> {
        if self.store_path.exists() {
            let content =
                std::fs::read_to_string(&self.store_path).map_err(|e| OrangeError::from(e))?;
            serde_json::from_str(&content)
                .map_err(|e| OrangeError::serialization(format!("密钥存储文件解析失败: {e}")))
        } else {
            // 文件不存在时返回空存储
    ...
    /// 将密钥存储保存到文件
    fn save_store(&self, store: &SecretStore) -> orangecoding_core::Result<()> {
        // 确保父目录存在
        if let Some(parent) = self.store_path.parent() {
            if !parent.exists() {
                std::fs::create_dir_all(parent)
                    .map_err(|e| OrangeError::io(format!("创建密钥存储目录失败: {e}")))?;
            }
        }

        let content = serde_json::to_string_pretty(store)
            .map_err(|e| OrangeError::serialization(format!("密钥存储序列化失败: {e}")))?;

        std::fs::write(&self.store_path, content).map_err(|e| OrangeError::from(e))?;
