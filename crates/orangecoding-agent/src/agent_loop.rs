...
impl AgentLoop {
    ...
    /// 失败时返回 `orangecoding_core::OrangeError`。
    pub async fn run(
        &mut self,
        chat_options: &ChatOptions,
        cancel_token: CancellationToken,
        event_sender: mpsc::Sender<AgentEvent>,
    ) -> orangecoding_core::Result<AgentLoopResult> {
    ...
    async fn call_ai_provider(
        ...
        options: &ChatOptions,
        cancel_token: &CancellationToken,
    ) -> orangecoding_core::Result<AiResponse> {
        ...
        tokio::select! {
            ...
            result = self.provider.chat_completion(messages, tools, options) => {
                result.map_err(|e| orangecoding_core::OrangeError::Ai {
            ...
            _ = cancel_token.cancelled() => {
                Err(orangecoding_core::OrangeError::Agent {
    ...
    /// 将对话中的 orangecoding-core 消息转换为 AI 提供者的 ChatMessage 格式
    fn convert_to_chat_messages(&self) -> Vec<ChatMessage> {
        self.context
            ...
            .map(|msg| {
                match msg.role {
                    orangecoding_core::message::Role::System => {
                        ChatMessage::system(msg.content.clone().unwrap_or_default())
                    }
                    orangecoding_core::message::Role::User => {
                        ChatMessage::user(msg.content.clone().unwrap_or_default())
                    }
                    orangecoding_core::message::Role::Assistant => {
                        if msg.tool_calls.is_empty() {
                            // 纯文本助手消息
                            ChatMessage::assistant(msg.content.clone().unwrap_or_default())
                        } else {
                            // 带工具调用的助手消息
                            let ai_tool_calls: Vec<orangecoding_ai::ToolCall> = msg
                                .tool_calls
                                .iter()
                                .map(|tc| orangecoding_ai::ToolCall {
                                    id: tc.id.clone(),
                    ...
                    orangecoding_core::message::Role::Tool => ChatMessage::tool_result(
    ...
    /// 将 AI 返回的工具调用转换为 orangecoding-core 的 ToolCall 格式
    fn convert_ai_tool_calls(&self, ai_calls: &[orangecoding_ai::ToolCall]) -> Vec<CoreToolCall> {
        ai_calls
            .iter()
            .map(|ac| {
        ...
        schemas_array
            ...
            .filter_map(|schema| {
                ...
                Some(ToolDefinition {
                    ...
                    function: orangecoding_ai::provider::FunctionDefinition {
                        name,
                        description,
                        parameters: orangecoding_ai::provider::ToolParameter {
...
mod tests {
    use super::*;
    use orangecoding_ai::{AiError, AiResult};
    use orangecoding_core::SessionId;
    ...
    #[async_trait::async_trait]
    impl orangecoding_tools::Tool for TestTool {
        fn name(&self) -> &str {
            "test_tool"
        }
