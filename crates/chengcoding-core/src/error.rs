//! 错误类型定义模块
//!
//! 本模块使用 `thiserror` 定义了 ChengCoding 系统的统一错误类型。
...
mod tests {
    ...
    fn 测试IO错误的自动转换() {
        ...
        let chengcoding_err: CeairError = io_err.into();
        let msg = format!("{chengcoding_err}");
    ...
    fn 测试JSON错误的自动转换() {
        ...
        let chengcoding_err: CeairError = json_err.into();
        let msg = format!("{chengcoding_err}");
