...
use ceair_tools::permissions::PermissionContext;
...
impl ToolExecutor {
    ...
    pub async fn execute_tool_call(&self, tool_call: &ToolCall) -> ToolResult {
        ...
        if let Some(ref ctx) = self.permission_ctx {
            use ceair_tools::permissions::PermissionDecision;
    ...
    pub async fn execute_batch(&self, tool_calls: &[ToolCall]) -> Vec<ToolResult> {
        ...
        use ceair_tools::batch_partition::{partition_tool_calls, ToolCallInfo};
...
mod tests {
    ...
    impl Tool for SafeCountTool {
        ...
        fn parameters_schema(&self) -> Value { json!({"type": "object", "properties": {}}) }
        fn metadata(&self) -> ceair_tools::ToolMetadata {
            ceair_tools::ToolMetadata::read_only()
        }
        async fn execute(&self, _params: Value) -> ceair_tools::ToolResult<String> {
    ...
    impl Tool for UnsafeCountTool {
        ...
        fn parameters_schema(&self) -> Value { json!({"type": "object", "properties": {}}) }
        fn metadata(&self) -> ceair_tools::ToolMetadata {
            ceair_tools::ToolMetadata::default()
        }
        async fn execute(&self, _params: Value) -> ceair_tools::ToolResult<String> {
            self.count.fetch_add(1, AtomicOrdering::SeqCst);
            Ok("unsafe".to_string())
