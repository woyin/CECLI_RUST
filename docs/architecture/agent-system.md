...
> ChengCoding 实现了一个多 Agent 协作系统，包含 11 种专业化 Agent，通过意图分类、类别路由和任务协商实现智能任务分配。
...
ChengCoding 的 Agent 系统位于 `chengcoding-agent` crate 中，是整个系统的核心。它实现了：
...
ChengCoding 定义了 11 种专业化 Agent，每种有独特的定位和默认模型：
