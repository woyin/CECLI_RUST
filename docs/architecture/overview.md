...
> CEAIR 是一个用 Rust 编写的多 Agent AI 编程助手系统，采用模块化 workspace 架构，由 11 个独立 crate 组成。
...
CEAIR（Code Engineering AI Runtime）是一个基于 Rust 的多 Agent AI 编程助手运行时系统。其设计目标是：
...
```
            ...
            ▼                          ▼                          ▼
   ┌─────────────────┐    ┌──────────────────┐    ┌─────────────────────┐
   │    ceair-tui     │    │   ceair-agent    │    │   ceair-config      │
   ...
   └────────┬────────┘    └────────┬─────────┘    └──────────┬──────────┘
            ...
            ▼                      ▼                          ▼
   ┌─────────────────┐    ┌──────────────────┐    ┌─────────────────────┐
   │   ceair-mesh    │    │   ceair-tools    │    │     ceair-ai        │
   ...
   └────────┬────────┘    └────────┬─────────┘    └──────────┬──────────┘
            ...
            ▼                      ▼              ▼
   ┌─────────────────┐    ┌──────────────┐   ┌──────────────────┐
   │  ceair-session   │    │  ceair-core  │   │   ceair-audit    │
   │  (会话管理)      │    │  (基础类型)   │   │  (审计日志)       │
   │                  │    │              │   │                  │
...
| `error.rs` | `CeairError` | 统一错误类型 |

**设计原则**：零外部 AI/网络依赖，仅使用 serde、chrono、uuid 等基础库。

### ceair-agent（Agent 核心循环）
...
| 模块 | 关键类型 | 描述 |
|------|----------|------|
| `config.rs` | `CeairConfig`, `AiConfig`, `AgentConfig`, `ToolsConfig`, `TuiConfig`, `LoggingConfig` | 配置数据结构 |
...
```
ceair-cli ──────┬──► ceair-tui
                ├──► ceair-agent ──┬──► ceair-tools ──► ceair-core
                ├──► ceair-config  │                      ▲
                └──► ceair-mcp     ├──► ceair-ai ─────────┘
                                   ├──► ceair-mesh ───────┘
                                   ├──► ceair-session ────┘
...
| 规则 | 描述 |
|------|------|
| `ceair-core` 无项目内依赖 | 基石层，被所有其他 crate 依赖 |
...
│  │                                                      │    │
│  │  ┌──── 网络边界 ──────────────────────────────────┐  │    │
│  │  │  ceair-ai (AI API 请求，HTTPS 强制)            │  │    │
│  │  │  ceair-mcp (MCP 通信，OAuth 认证)              │  │    │
│  │  └────────────────────────────────────────────────┘  │    │
│  │                                                      │    │
│  │  ┌──── 存储边界 ──────────────────────────────────┐  │    │
│  │  │  ceair-config (加密密钥存储，AES-256)          │  │    │
│  │  │  ceair-session (会话文件，用户权限)             │  │    │
│  │  │  ceair-audit (审计日志，只追加)                 │  │    │
│  │  └────────────────────────────────────────────────┘  │    │
│  │                                                      │    │
│  └──── 用户空间 ────────────────────────────────────────┘    │
│                                                              │
