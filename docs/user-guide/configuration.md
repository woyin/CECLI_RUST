...
> ChengCoding 使用 JSONC（带注释的 JSON）格式进行配置，支持项目级和全局级的层级覆盖。
...
ChengCoding 使用 JSONC（JSON with Comments）格式，在标准 JSON 基础上支持：
...
| 级别 | 路径 | 说明 |
|------|------|------|
| **全局配置** | `~/.ChengCoding/config.jsonc` | 用户级全局设置 |
| **项目配置** | `.opencode/ChengCoding.jsonc` | 项目级设置 |
...
```
内置默认值 < 全局配置 (~/.ChengCoding/config.jsonc) < 项目配置 (.opencode/ChengCoding.jsonc)
```

### 适合放在全局配置中的设置

```jsonc
// ~/.ChengCoding/config.jsonc
{
  // API Key 通常是全局的
  "providers": {
    "openai": {
...
ChengCoding 使用模型角色系统为不同类型的任务选择合适的模型：
...
ChengCoding 的 Intent Gate 系统自动分类用户意图：
...
```jsonc
{
  "hooks": {
    ...
    "post_message": [
      {
        "priority": 20,
        "script": ".ChengCoding/hooks/log-message.sh",
        "timeout": 5                     // 脚本超时（秒）
      }
    ],
    ...
    "pre_tool_call": [
      {
        "priority": 1,                  // 最高优先级
        "script": ".ChengCoding/hooks/security-check.sh",
...
`.ChengCoding/hooks/security-check.sh`：
...
|------|--------|------|
| 内置 | 最低 | 程序内部 |
| 用户全局 | 中 | `~/.ChengCoding/skills/` |
| 项目 | 最高 | `.ChengCoding/skills/` |
...
```jsonc
{
  "logging": {
    ...
    "file": "~/.ChengCoding/logs/ChengCoding.log",   // 日志文件路径
...
```jsonc
// .opencode/ChengCoding.jsonc — 完整配置示例
{
  // === AI 提供者配置 ===
  "ai": {
    "provider": "anthropic",
