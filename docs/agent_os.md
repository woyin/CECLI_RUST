...
> ChengCoding Agent OS 是一个 AI 驱动的软件工程系统，具备**学习、验证、防护、修复、进化**五大核心能力。它不仅是一个代码生成工具，更是一个拥有完整生命周期管理的智能操作系统。
...
#### 2.1.3 会话管理 — `chengcoding-session`
...
> 路径：`crates/chengcoding-invariant/src/rules.rs`, `checker.rs`, `report.rs`
...
> 路径：`crates/chengcoding-invariant/src/gate.rs`, `runtime_guard.rs`, `rollback.rs`
...
> 路径：`crates/chengcoding-invariant/src/verification.rs`
...
> 路径：`crates/chengcoding-mesh/src/`, `crates/chengcoding-worker/src/`

#### 2.5.1 任务编排 — `chengcoding-mesh`
...
#### 2.5.2 Worker 运行时 — `chengcoding-worker`
...
> 路径：`crates/chengcoding-agent/src/`
...
> 路径：`crates/chengcoding-invariant/src/healing.rs`, `evolution.rs`
...
> 路径：`crates/chengcoding-control-protocol/src/`, `crates/chengcoding-control-server/src/`, `crates/chengcoding-worker/src/`

#### 2.8.1 控制协议 — `chengcoding-control-protocol`
...
#### 2.8.2 控制服务器 — `chengcoding-control-server`
...
用户输入
  ↓
AgentLoop (chengcoding-agent)
  ↓
AI 模型调用 (chengcoding-ai)
  ↓
工具调用请求
  ↓
...
浏览器 UI
  ↓ WebSocket
ControlServer (chengcoding-control-server)
  ↓ ClientCommand
WorkerRuntime (chengcoding-worker)
...
| 能力 | 模块 | 关键类型 | 状态 |
|------|------|---------|------|
| 不变量定义 | `chengcoding-invariant/rules.rs` | `InvariantRule` (18 条) | ✅ 已完成 |
| 运行时检查 | `chengcoding-invariant/checker.rs` | `InvariantChecker` | ✅ 已完成 |
| 违规报告 | `chengcoding-invariant/report.rs` | `ViolationReport` | ✅ 已完成 |
| 预检门控 | `chengcoding-invariant/gate.rs` | `PreCheckGate` | ✅ 已完成 |
| 运行时守卫 | `chengcoding-invariant/runtime_guard.rs` | `RuntimeGuard` | ✅ 已完成 |
| 自动回滚 | `chengcoding-invariant/rollback.rs` | `AutoRollback` | ✅ 已完成 |
| 验证代理 | `chengcoding-invariant/verification.rs` | `VerificationAgent` | ✅ 已完成 |
| 自我修复 | `chengcoding-invariant/healing.rs` | `SelfHealer` | ✅ 已完成 |
| 自我进化 | `chengcoding-invariant/evolution.rs` | `EvolutionEngine` | ✅ 已完成 |
| 工具系统 | `chengcoding-tools` | `Tool` trait (24+ 工具) | ✅ 已完成 |
| 会话管理 | `chengcoding-session` | `SessionManager` | ✅ 已完成 |
| 审计追踪 | `chengcoding-audit` | `AuditLogger` + `HashChain` | ✅ 已完成 |
| MCP 协议 | `chengcoding-mcp` | `McpServer` / `McpClient` | ✅ 已完成 |
| 多代理编排 | `chengcoding-mesh` | `TaskOrchestrator` | ✅ 已完成 |
| 代理执行 | `chengcoding-agent` | `AgentLoop` | ✅ 已完成 |
| 本地 Web 控制面 | `chengcoding-control-server` | `ControlServer` | ✅ Phase A |
| 远程 Worker 连接 | `chengcoding-worker` | `WorkerRuntime` | 🔄 Phase B |
...
```text
                    ...
                    ↓           ↓           ↓
             ┌──────────┐ ┌─────────┐ ┌─────────┐
             │ chengcoding-tui │ │chengcoding-mcp│ │chengcoding-cfg│
             └────┬─────┘ └────┬────┘ └─────────┘
                  │            │
                  ↓            ↓
           ┌────────────────────────────────────┐
          ...
          ↓           ↓            ↓
    ┌──────────┐ ┌──────────┐ ┌──────────┐
    │chengcoding-tool│ │chengcoding-mesh│ │ chengcoding-ai │
    │ 24+ tools│ │Orchestr. │ │ModelProxy│
    └────┬─────┘ └────┬─────┘ └──────────┘
         │            │
         ↓            ↓
    ┌──────────┐ ┌──────────────────┐
    │chengcoding-sess│ │  chengcoding-invariant  │
    ...
    └────┬─────┘ │ Gate · Guard      │
         ...
         ↓                ↓
    ┌──────────┐    ┌──────────┐
    │chengcoding-audi│    │chengcoding-core│
    │ HashChain│    │ Event    │
    │ Sanitizer│    │ Types    │
    └────┬─────┘    └──────────┘
         │                ↑
