...
影响范围：
- `crates/ceair-control-server/src/routes.rs` — 路由配置
- `crates/ceair-control-server/src/ws.rs` — WebSocket handler
- `crates/ceair-control-server/src/auth.rs` — LocalAuth（已存在但未用于 WS）
...
影响范围：
- `crates/ceair-cli/src/commands/serve.rs` — LocalAgentExecutor
- `crates/ceair-worker/src/session_bridge.rs` — SessionSupervisor（token 管理正确）
- `crates/ceair-agent/src/agent_loop.rs` — AgentLoop::run（已支持 CancellationToken）
- `crates/ceair-worker/src/runtime.rs` — AgentExecutor trait
...
影响范围：
- `crates/ceair-cli/src/commands/serve.rs` — LocalAgentExecutor（根因）
- `crates/ceair-agent/src/context.rs` — AgentContext
- `crates/ceair-worker/src/session_bridge.rs` — SessionSupervisor（需扩展存储 context）
